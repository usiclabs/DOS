    ...
    for (const token of tokensToProcess) {
      try {
        ...
        if (token.address.toLowerCase() === DEUS_TOKEN_ADDRESS.toLowerCase()) {
          // During build time, we can't reliably construct absolute URLs
          // So we'll just use Dexscreener as the primary source for DEUS price
          price = await getTokenPrice(token.address)
          console.log("[v0] DEUS price from Dexscreener:", price)
        } else {
          price = await getTokenPrice(token.address)
        }
